...
def _get_language_dict(skin_dict, config_dict):
    ...
    if 'Language' in skin_dict:
        if 'language' in skin_dict['Language']:
            ...
            try:
              #syslog.syslog(syslog.LOG_INFO, "%s: language_config_path is %s" % (os.path.basename(__file__), language_config_path))
              language_dict = ConfigObj(language_config_path)
            except:
              #syslog.syslog(syslog.LOG_INFO, "%s: Could not import lanuguage dictionary %s" % os.path.basename(__file__), language_config_path)
              language_dict = None
